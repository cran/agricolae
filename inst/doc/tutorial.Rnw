...
For a complete functionality, \textbf{agricolae} requires other packages

\textbf{MASS:}  for the generalized inverse used in the function \emph{PBIB.test}\\
\textbf{nlme:}  for the methods REML and LM in \emph{PBIB.test}\\
\textbf{klaR:}  for the function \emph{triplot} used in the function \emph{AMMI}\\
\textbf{Cluster:}  for the use of the function \emph{consensus}\\
\textbf{algDesign:}  for the balanced incomplete block design\emph{design.bib}

\subsection{Use in \textbf{R} }

Since \textbf{agricolae }  is a package of functions, these are operational when they are called directly from the console of \textbf{R}  and are integrated to all the base functions of \textbf{R}.
...
\subsection{Data set in \textbf{agricolae }}
<<>>=
...
The package \textbf{agricolae}  provides some complementary functions to the \textbf{R}  program, specifically for the management of the histogram and function \emph{hist}.
...
Example.  Data generated in \textbf{R} . (students' weight).
...
Limits class: \textbf{Lower and  Upper}

Class point: \textbf{Main}

Frequency: \textbf{Frequency}

Percentage frequency: \textbf{Percentage}

Cumulative frequency: \textbf{CF}

Cumulative percentage frequency: \textbf{CPF}
...
The package \textbf{agricolae}  presents special functions for the creation of the field book for experimental designs. Due to the random generation, this package is quite used in agricultural research. 

For this generation, certain parameters are required, as for example the name of each treatment, the number of repetitions, and others, according to the design,  \citet{CochCox:1992}; \citet{kueh:2000}; \citet{LeClLeonErwiWarrAndre:1962}; \citet{Mont:2002}. There are other parameters of random generation, as the seed to reproduce the same random generation or the generation method (See the reference manual of \textbf{agricolae}).

\url{http://cran.at.r-project.org/web/packages/agricolae/agricolae.pdf}

\textbf{Important parameters in the generation of design:}

\textbf{series:} A constant that is used to set numerical tag blocks , eg number = 2, the labels will be : 101, 102, for the first row or block, 201, 202, for the following , in the case of completely randomized design, the numbering is sequencial.

\textbf{design:} Some features of the design requested agricolae be applied specifically to design.ab(factorial) or design.split (split plot) and their possible values are: "rcbd", "crd" and "lsd".

\textbf{seed:} The seed for the random generation  and its value is any real value, if the value is zero, it has no reproducible generation, in this case copy of value of the outdesign\$parameters.

\textbf{kinds:} the random generation method, by default "Super-Duper".

\textbf{first:} For some designs is not required random the first repetition, especially in the block design, if you want to switch to random, change to TRUE.

\textbf{randomization:} TRUE or FALSE. If false, randomization is not performed

\textbf{Output design:}

\textbf{parameters:} the input to generation design, include the seed to generation random, if seed=0, the program generate one value and it is possible reproduce the design.

\textbf{book:} field book

\textbf{statistics:} the information statistics the design for example efficiency index, number of treatments.

\textbf{sketch:} distribution of treatments in the field.

\textbf{The enumeration of the plots}
...
\textbf{fieldbook:} output zigzag, contain field book.
...
These designs have two factors, one is applied in plots and is defined as \textbf{trt1}  in a randomized complete block design; and a second factor as  \textbf{trt2} , which is applied in the subplots of each plot applied at random. The function design.split() permits to find the experimental plan for this design and its parameters are:
...
The full factorial of n factors applied to an experimental design (CRD, RCBD and LSD) is common and this procedure in \textbf{agricolae} applies the factorial to one of these three designs and its parameters are:
...
Generate the design matrix from the field book generated by an experimental plan or a data table for analysis and its parameters are:

<<>>= 
str(design.mat)
@
To generate the matrix, it is necessary to extract the field book of the design generated by agricolae. The matrix can also be generated if you have a table for experimental analysis.
...
print(X)
@
Matrix X is of incomplete range, to find an estimate, the restriction of condition of the effects is added, for example the weighted sum for repetitions of the effects is zero to the matrix X and thus it is complete, in this way the estimators are found. 
<<>>=
...
@
\textbf{Unadjusted sums of squares}
<<>>=
SCtot<-t(Y)%*%Y 
SCb<-t(beta)%*%XX%*%beta
SCe<-t(e)%*%e
...
print(SS)
@
\textbf{Adjusted sum of squares and mean square}
<<>>=
trt<-tau
t<-length(trt)
XX1<-XX[2:5,2:5]
...
For the analyses, the following functions of \textbf{agricolae} are used: {\emph LSD.test,  HSD.test, duncan.test, scheffe.test,  waller.test, SNK.test, REGW.test}, \citet{SteeTorrDick:1997}; \citet{Hsu:1996} and {\emph durbin.test, kruskal, friedman,  waerden.test and Median.test}, \citet{Cono:1999}.

For every statistical analysis, the data should be organized in columns. For the demonstration, the \textbf{agricolae}  database will be used.
...
\textbf{Model parameters: Degrees of freedom and variance of the error:} 
...
With the function \emph{LSD.test} we can make adjustments to the probabilities found, as for example the adjustment by Bonferroni, holm and other options see Adjust P-values for Multiple Comparisons, function p.adjust(stats), \citet{Rcore:2018}.
...
\subsection{Tukey (HSD) for different repetition}

Include the argument \textbf{unbalanced = TRUE}  in the function. Valid for group = TRUE/FALSE
...
\textbf{The comparison is the combination of A:B:C.}
...
This analysis can come from balanced or partially balanced designs. The function {\emph BIB.test} is for balanced designs, and {\emph BIB.test}, for partially balanced designs. In the following example, the \textbf{agricolae} data will be used, \citet{Josh:1987}.
...
\textbf{function (block, trt, Y, test = c("lsd", "tukey", "duncan",  "waller", "snk"), alpha = 0.05, group = TRUE)} LSD, Tukey Duncan, Waller-Duncan and SNK, can be used. The probabilities of the comparison can also be obtained. It should only be indicated: group=FALSE, thus:
...
\textbf{bar.group:} out\$groups

\textbf{bar.err:}  out\$means
...
\textbf{The adjusted averages can be extracted from the modelPBIB.}

head(modelPBIB\$means)

\textbf{The comparisons:} 
...
\textbf{The adjusted averages can be extracted from the modelLattice.}
...
\textbf{The treatments are in each block:}

<<>>=
by(trt,block,as.character)
@

\textbf{With their respective responses:}
...
The functions for non-parametric multiple comparisons included in \textbf{agricolae}  are: \emph{kruskal, waerden.test, friedman and durbin.test}, \citet{Cono:1999}. 
...
Included in the \textbf{agricolae}  package 
...
\textbf{For the examples, the \textbf{agricolae}  package data will be used} 
...
\subsection {Waerden}
A nonparametric test for several independent samples. Example applied with the sweet potato data in the \textbf{agricolae}  basis.
...
The comparison probabilities are obtained with the parameter group = \textbf{FALSE} 
<<>>=
...
In \textbf{agricolae}  there are two methods for the study of stability and the AMMI model. These are: a parametric model for a simultaneous selection in yield and stability "SHUKLA'S STABILITY VARIANCE AND KANG'S",  \citet{Kang:1993} and a non-parametric method of Haynes, based on the data range.
...
For the application, we consider the data used in the example of parametric stability (study):
  ...
  \textbf{AMMI structure}
<<>>=
  str(AMMI)
@
  \textbf{plot.AMMI structure, plot()}
<<>>=
  str(plot.AMMI)
@
  \textbf{type: 1=biplot, 2= triplot} 
<<>>=
data(plrv)
...
\textbf{To triplot require klaR package. in R execute:}
...
\textbf{Construct a classic dendrogram, execute procedure in R}
...
@
\textbf{Some statistics, original data:}
<<>>=
summary(soil$pH)
@
\textbf{Some statistics, montecarlo simulate data:}
<<>>=
summary(simulated)
@
...
\textbf{Example with the heterosis data, locality 2.} 
...
\textbf{Application with 'agricolae' data:}
...
\underline{coefficient of variation.}
<<>>=
data(sweetpotato)
...
\textbf{skewness is calculated with:}

<<>>=
skewness(x)
@

\textbf{and kurtosis with:}
...
\textbf{Function of Waller to different value of parameters K and Fc} 
...
\textbf{Generating table Waller-Duncan} 
...
\bibitem[\protect\citeauthoryear{R Core Team}{R Core Team}{2020}]{Rcore:2020} 
R Core Team (2020).
\newblock \emph {A language and environment for statistical computing.}
\newblock \emph {R Foundation for Statistical Computing}
\newblock Vienna, Austria. \url{http://www.R-project.org}
