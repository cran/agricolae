...
@
\caption{Combined clone:nitrogen\label{fig:f4}}
\end{center}
\end{figure}
...
detach(corn)
@
The object output has the same structure of the comparisons see Figure ~\ref{fig:f9}.
...
str(bar.group)
@
The found object of one comparison is the entry for these functions, see Figure ~\ref{fig:f5}. 
...
\subsection{bar.err}
A function to plot horizontal or vertical bar, where the variation of the error is expressed in every treatments. The function applies to all functions comparison treatments. Each object must use the means object previously generated by the comparison function.
<<>>=
# model <-aov (yield ~ fertilizer, data = field) 
# out <-LSD.test (model, "fertilizer", group = TRUE) 
# bar.err(out$means)
...
main="Standard error \nG4",las=1)
@
\caption{Comparison between treatments\label{fig:f5}}
\end{center}
\end{figure}
\begin{figure}[!hbtp]
\begin{center}

<<f6, fig=TRUE,width=6, height=2.5>>=
par(cex=0.8,mar=c(4,3,1,0))
G5<-bar.group(modelPBIB$groups,border="brown",col="white",ylim=c(0,9),las=2)
@
\caption{Treatment Groups\label{fig:f6}}
\end{center}
\end{figure}
\begin{figure}[!hbtp]
\begin{center}

<<f7, fig=TRUE,width=6, height=3.2>>=
...
@
\caption{Range in each treatment\label{fig:f7}}
\end{center}
\end{figure}

\begin{figure}[!hbtp]
\begin{center}
<<f8, fig=TRUE,width=4, height=1.2>>=
par(mar=c(2.5,2.5,1,0),cex=0.6)
#
bar.group(modelLattice$group,ylim=c(0,55),density=10,las=1)
@
\caption{Treatment Groups Lattice\label{fig:f8}}
\end{center}
\end{figure}

\begin{figure}[!hbtp]
\begin{center}
<<f9, fig=TRUE,width=2.5, height=1.5>>=
par(mar=c(2.5,2.5,1,0),cex=0.6)
bar.group(outKruskal$groups,ylim=c(0,40),col=colors()[55],las=1)
@
\caption{Comparison according to Kruskal-Wallis\label{fig:f9}}
\end{center}
\end{figure}
...
When performing AMMI, this generates the Biplot, Triplot and Influence graphics, see Figures ~\ref{fig:f10}, ~\ref{fig:f11}
...
\begin{figure}[!hbtp]
\begin{center}
<<f10, fig=TRUE,width=6, height=2.4>>=
require(klaR)
par(mfrow=c(1,2),cex=0.8,mar=c(4,4,1,0))
plot(model,type=1,las=1)
plot(model,type=2,las=1)
@
\caption{Biplot and Triplot\label{fig:f10}}
\end{center}
\end{figure}
In this case, the interaction is significant. The first two components explain 67.8\%; then the biplot can provide information about the interaction genotype-environment. With the triplot, 90.4\% would be explained.
\begin{figure}[!hbtp]
\begin{center}
<<f11, results=hide,fig=TRUE,width=2.5, height=1.5>>=
require(spdep)
par(cex=0.5,mar=c(3,3,1,0))
plot(model,type=3,las=1)
@
\caption{Influence genotype\label{fig:f11}}
\end{center}
\end{figure}
...
The program identifies duplicates in the rows and can operate in both cases. The result is a dendrogram, in which the consensus percentage is included, see Figure ~\ref{fig:f12}.

\begin{figure}[!hbtp]
\begin{center}
<<f12, fig=TRUE,width=5, height=2>>=
...
@
\caption{Dendrogram, production by consensus\label{fig:f12}}
\end{center}
\end{figure}
When the dendrogram is complex, it is convenient to extract part of it with the function hcut(), see Figure ~\ref{fig:f13}.

\begin{figure}[!hbtp]
\begin{center}
<<f13, fig=TRUE,width=5, height=2>>=
...
@
\caption{Dendrogram, production by hcut()\label{fig:f13}}
\end{center}
\end{figure}
...
names(output)
@
This means that we can know the duplicates, reconstruct the tree diagram and maintain the interactions, see Figure ~\ref{fig:f14}.
\bf {repeating dendrograms:}\rm 
\begin{figure}[!hbtp]
\begin{center}
<<f14, fig=TRUE,width=5, height=2>>=
...
@
\caption{Classic dendrogram \label{fig:f14}}
\end{center}
\end{figure}
\bf {Construct a classic dendrogram, see Figure ~\ref{fig:f15}\rm
\begin{figure}[h]
\begin{center}

<<f15, fig=TRUE,width=4, height=2>>=
...
@
\caption{Better classic dendrogram \label{fig:f15}}
\end{center}
\end{figure}
<<>>=
...
The probability density of the original and simulated data can be compared, see Figure ~\ref{fig:f16}.
<<>>=
data(soil)
# set.seed(9473)
...
\begin{figure}[!hbtp]
\begin{center}
<<f16, fig=TRUE, width=4, height=1.5>>=
...
@
\caption{Distribution of the simulated and the original data\label{fig:f16}}
\end{center}
\end{figure}
...
For the test, we will use the rice file. The graphic is a result with the adjustment of a model for the parcel size and the coefficient of variation, see Figure ~\ref{fig:f17}.
\begin{figure}[h]
\begin{center}
<<f17, fig=TRUE, width=6, height=3>>=
...
@
\caption{Adjustment curve for the optimal size of plot\label{fig:f17}}
\end{center}
\end{figure}
<<>>=
...
\bf {Factor and variable table}\rm

\bf {Application with 'agricolae' data:}\rm
<<>>=
data(RioChillon)
...
The area under the disease progress curve (AUDPC), see Figure ~\ref{fig:f18} calculates the absolute and relative progress of the disease. It is required to measure the disease in percentage terms during several dates, preferably equidistantly.
<<>>=
...
\begin{figure}[!hbtp]
\begin{center}
<<f18, echo=FALSE, fig=TRUE, width=4, height=2>>=
...
@
\caption{AUDPC: Area under the curve\label{fig:f18}}
\end{center}
\end{figure}
...
LATEBLIGHT is a mathematical model that simulates the effect of weather, host growth and resistance, and fungicide use on asexual development and growth of Phytophthora infestans on potato foliage, see Figure ~\ref{fig:f19}
...
\begin{figure}[h]
\begin{center}
<<f19, fig=TRUE, width=4.5, height=2.5>>=
par(mar=c(3,3,4,0),cex=0.7)
#--------------------------
...
@
\caption{lateblight: LATESEASON\label{fig:f19}}
\end{center}
\end{figure}
\newpage
<<>>=
