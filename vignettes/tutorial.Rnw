...
For this generation, certain parameters are required, as for example the name of each treatment, the number of repetitions, and others, according to the design references \cite{b1,b6,b7,b8}. There are other parameters of random generation, as the seed to reproduce the same random generation or the generation method (See the reference manual of \bf{agricolae} \rm.
...
For the analyses, the following functions of \bf{agricolae} \rm are used: \emph{LSD.test,  HSD.test, duncan.test, scheffe.test,  waller.test, SNK.test, REGW.test} reference \cite{b11, b17} and \emph{durbin.test, kruskal, friedman,  waerden.test and Median.test} reference \cite{b2}.
...
Multiple range tests for all pairwise comparisons, to obtain a confident inequalities multiple range tests. Reference \cite{b17}
...
It plots bars of the averages of treatments to compare. It uses the objects generated by a procedure of comparison like LSD (Fisher), duncan, Tukey (HSD), Student Newman Keul (SNK), Scheffe, Ryan, Einot and Gabriel and Welsch (REGW), Kruskal Wallis, Friedman and Waerden. Reference \cite{b17}, see Figure ~\ref{fig:f5}
...
\bibitem{b17}Hsu J. C., 1996. \emph{Multiple Comparison, theory and methods}. First Edition. CRC Press reprint 1999. Printed in the United States of America.
