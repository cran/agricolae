...
The main program of \bf {R} \rm should be already installed in the platform of your computer \emph{(Windows, Linux or MAC)}. If it is not installed yet, you can download it from the R project \emph{(www.r-project.org)} of a repository CRAN, \citet{Rcore:2018}.
...
Include the argument \bf{unbalanced = TRUE} \rm in the function. Valid for group = TRUE/FALSE
