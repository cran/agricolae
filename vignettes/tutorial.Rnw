...
The main program of \textbf{R} should be already installed in the platform of your computer \emph{(Windows, Linux or MAC)}. If it is not installed yet, you can download it from the R project \emph{(www.r-project.org)} of a repository CRAN, \citet{Rcore:2018}.
...
\textbf{spdep:}  for the between genotypes spatial relation in biplot of the function \emph{AMMI}\\
...
<<>>=
  str(plot.AMMI)
@
  \textbf{type: 1=biplot, 2= triplot 3=influence genotype} 
<<>>=
data(plrv)
...
\textbf{To Influence graphics genotype require spdep package, in R execute:}
...
\bibitem[\protect\citeauthoryear{R Core Team}{R Core Team}{2018}]{Rcore:2018} 
R Core Team (2017).
